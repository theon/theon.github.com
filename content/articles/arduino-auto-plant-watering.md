...
[![alt text](https://lh4.googleusercontent.com/-vXV0TmxhJOQ/UFrC4OKp4YI/AAAAAAAACkg/ZXUOlqbvis0/s800/DSC_0004.JPG)](https://lh4.googleusercontent.com/-vXV0TmxhJOQ/UFrC4OKp4YI/AAAAAAAACkg/ZXUOlqbvis0/s1280/DSC_0004.JPG)
...
The x-axis is time - one pixel per 5 minutes. The y-axis is moisture level in an arbritary unit - the number is actually a reading of one of the arduino's analogue pins which has 10 bits of resolution, so it has the theoretical range of `0` - `1023`. Saying that if the chart ever reads `0`, then my chilli has either been long dead, or I have hi-jacked the arduino for my next project. A normal reading should be somewhere in the range of `500` - `800`.
...
Below is the circuit diagram for driving the solenoid valve. My electronics knowledge is poor, however I managed to cobble this together from various reasearch and it seems to work. The arduino's VIN pin gives you access to the the input voltage when using a mains adapter to power the arduino. I used a 12V 1500mA adapter I had in the flat. The diode in parallel with the solenoid is there as a [snubber dioide](http://en.wikipedia.org/wiki/Flyback_diode).  
